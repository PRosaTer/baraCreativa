...
import {
  ...
  TipoCurso,
  ModalidadCurso,
} from "@/app/types/shared-backend-types";
...
const CrearCursoMultiStep: React.FC<Props> = ({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const courseDataForApi: Omit<
        Curso,
        "id" | "modulos" | "videoCurso" | "pdfCurso"
      > & { modulos?: ModuloDto[] } = {
        titulo: form.titulo,
      ...
      if (curso) {
        const resUpdate = await fetch(
          `http://localhost:3001/api/cursos/${curso.id}`,
          {
            method: "PATCH",
      ...
      if (form.newScormFile) {
        ...
        const resScorm = await fetch(
          "http://localhost:3001/api/cursos/scorm_unzipped_courses",
          {
            method: "POST",
            body: formDataScorm,
          }
      ...
      if (form.imagenCurso && form.imagenCurso instanceof File) {
        ...
        const resImg = await fetch(
          `http://localhost:3001/api/cursos/${nuevoCurso.id}/imagen`,
          {
            method: "POST",
            body: formDataImagen,
          }
    ...
    <form onSubmit={handleSubmit} style={formStyle}>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        ...
        <button
          type="button"
          onClick={onCancelar}
          style={{ ...buttonStyle, flex: 1, backgroundColor: "#6b7280" }}
          disabled={loading}
        >
          Cancelar
        </button>
